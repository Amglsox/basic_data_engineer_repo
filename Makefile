...
start-airflow: ## Start Airflow
	docker build . -f ./airflow/Dockerfile --pull --tag airflow_databricks:latest
...
.PHONY: build-api
build-api: ## Build API
	cp poetry.lock pyproject.toml /api/acerto_data_challenge/ && cd ./api/acerto_data_challenge/ && docker build . -t data-challenge-api:latest && rm -rf poetry.lock pyproject.toml

.PHONY: start-container-api
start-container-api: ## Start API
	docker run --name data-challenge-api -v data:/code/data --network airflow_default -d -p 7979:7979 data-challenge-api

.PHONY: down-container-api
down-container-api: ## Start API
	docker stop data-challenge-api && docker rm data-challenge-api

.PHONY: deploy-solution
deploy-solution: build-api start-airflow start-container-api

.PHONY: down-solution
down-solution: down-container-api down-airflow